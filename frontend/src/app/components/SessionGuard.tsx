...
const TIMEOUT_MS    = 30 * 60 * 1000;  // 30 min inatividade (padrão financeiro)
const WARNING_MS    = 5  * 60 * 1000;  // aviso 5 min antes
const POLL_MS       = 30 * 1000;       // verifica a cada 30s
...
export default function SessionGuard({ children }: { children: React.ReactNode }) {
  ...
  const startCountdown = useCallback((remainingMs: number) => {
    if (countdownRef.current) return; // já está contando
          ...
          <div className="bg-[#1a1a2e] border border-amber-500/30 rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4 text-center">
            ...
            <div className="flex flex-col gap-2">
              ...
              <button
                ...
                className="w-full py-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-white/60 hover:text-white text-sm transition-all"
              >
                {t("sess_disconnect")}
              </button>
            </div>
